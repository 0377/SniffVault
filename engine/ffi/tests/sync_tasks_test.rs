...
#[test]
fn enqueue_single_with_auth_list_tasks_omits_cookie() {
    let dir = tempdir().unwrap();
    let path = CString::new(dir.path().to_str().unwrap()).unwrap();
    let handle = unsafe { engine_open(path.as_ptr()) };
    assert!(!handle.is_null());

    let title = CString::new("authed").unwrap();
    let url = CString::new("https://example.com/video.mp4").unwrap();
    let opts = CString::new(r#"{"cookies":"sid=ok","referer":"https://x/page"}"#).unwrap();
    let result = unsafe {
        engine_enqueue_single(
            handle,
            title.as_ptr(),
    ...
    assert_eq!(parsed["ok"], true);
    assert!(
        !json_str.contains("sid=ok"),
        "list_tasks JSON must not leak cookie snapshot: {json_str}"
    );
